...
if (localStorage["disable-ads"] === "true") {
    ...
    style.innerHTML =
        '[data-widget-definition="xa_ams_latest_articles"],[data-position="big_main_banner"],.p-body-sidebar,.samBannerUnit,img#register {display: none !important}';
...
function getThread() {
    ...
    data.append(
        '<style>[data-widget-definition="xa_ams_latest_articles"],[data-position="big_main_banner"],.p-body-sidebar,.block.block--messages .block-outer,form,.blockMessage.blockMessage--none,.p-breadcrumbs--bottom {display: none !important}</style>'
    );
    $('<span class="globalAction"><span class="globalAction-bar" /></span>').appendTo("body").addClass("is-active");
    ...
    function completeScraping() {
        $(".globalAction").remove();
        data.find(
            "footer.p-footer:not(:last),header.p-header:not(:first),.p-navSticky:not(:first),.p-sectionLinks:not(:first),.p-breadcrumbs:not(:first),.p-body-header:not(:first)"
        ).remove();
